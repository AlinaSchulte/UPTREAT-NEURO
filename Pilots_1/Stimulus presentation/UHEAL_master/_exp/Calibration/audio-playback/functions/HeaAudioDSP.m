...
classdef HeaAudioDSP < HeaAudio
    ...
    properties (Hidden = true,Access = protected)
        ...
        haf = []; %handle to the dsp.MatFileReader object
        filename = 'temp.mat'; %temporary file name for the haf
        isConstructed;
    end
    ...
    methods
        ...
        function play(obj,signalToPlay)
            ...
            %Create temporary .mat file with the data
            save(obj.filename,'signalToPlay','-v7.3'); % Write to MAT file
            
            %Create the handle to the dsp.MatFileReader
            obj.haf = dsp.MatFileReader(obj.filename,'VariableName',...
                'signalToPlay','SamplesPerFrame', obj.samplesPerFrame);
            
            %Play audio
            fprintf('DSP audioplayer utility running... ')
            while ~isDone(obj.haf)
                audio = step(obj.haf);
                step(obj.hap,audio);
            end
            pause(obj.hap.QueueDuration); % wait until audio is played to the end
            release(obj.haf);         % close the input file
            release(obj.hap);
        ...
        function signalRecorded = playrec(obj,signalToPlay)
            ...
            % If recNbSamples is not a multiple of the buffer size, there will be more
            % samples recorded than asked bu the user
            signalRecorded = signalRecorded(1:size(signalToPlay,1),:);
